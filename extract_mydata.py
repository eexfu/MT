...
if __name__ == '__main__':
    window_length_list = [1000, 1000, 500, 200, 100]
    step_pre_t0_list = [1000, 1000, 500, 200, 100]
    step_mid_t0_list = [250, 250, 125, 50, 25]
    step_post_t0_list = [150, 150, 75, 30, 15]
    mid_window_length = [1200, 1000, 1000, 1000, 1000]
    num_class = 6
    ...
    for i in range(len(window_length_list)):
        ...
        processor = AudioProcessor(
            ...
            mid_window_length=mid_window_length[i],
            image_width=1936,
            num_class=num_class
        )
            ...
            for resolution in resolution_list:
                for freq_range in freq_range_list:
                    ...
                    extract_from_wav_folder(
                        ...
                        save_path=rf".\preprocess_mydata\metadata_samples_L{L}_r{resolution}_f{fmin}-{fmax}_w{window_length_list[i]}_pre{step_pre_t0_list[i]}_post{step_post_t0_list[i]}_mid{step_mid_t0_list[i]}_mw{mid_window_length[i]}_c{num_class}.csv",
                        mic_array=mic_array,
